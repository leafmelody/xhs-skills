...
> **基于项目**：[autoclaw-cc/xiaohongshu-skills](https://github.com/autoclaw-cc/xiaohongshu-skills)  
> 本仓库是在上游项目基础上继续开发的二次实现，核心改动是将 Chrome Extension Bridge 架构迁移为 **BitBrowser CDP Python** 实现，以适配无头/自动化采集场景。
...
git clone https://github.com/leafmelody/xhs-skills.git
```
...
1. **BitBrowser 已启动**（默认 API 端口 54345）
2. **BitBrowser 中已登录小红书账号**

安装完成后即可使用 — 所有操作都通过 BitBrowser 的 CDP 连接在你的真实浏览器环境中执行。
...
> 确保 BitBrowser 已启动并登录小红书后再运行 CLI。
...
│   ├── xhs/                        # 核心自动化包（基于 BitBrowser CDP）
│   │   ├── bitbrowser_cdp.py       # BitBrowser CDP 连接适配器
...
[![Star History Chart](https://api.star-history.com/image?repos=leafmelody/xhs-skills&type=date&legend=top-left)](https://www.star-history.com/?repos=leafmelody%2Fxhs-skills&type=date&legend=top-left)
